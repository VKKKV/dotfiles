...
#      if [[ "$window_title" == *"(Bitwarden - Free Password Manager) - Bitwarden"* ]]; then
      if [[ "$window_title" == *"(Bitwarden - Free Password Manager) - Bitwarden"* ]] || \
         [[ "$window_title" == *([Unofficial] Saladict - Pop-up Dictionary) - 沙拉查词-独立查词窗口* ]] || \
         [[ "$window_title" == *"Yet Another Program Title"* ]]; then
