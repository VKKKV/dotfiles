...
return {
    ...
    -- .cast -> ((Type) expr)
    postfix({ trig = ".cast", match_pattern = text_pattern }, fmt("(({}) {})", {
        i(1, "Type"),
        match_content(2)
    })),

    -- .sout -> System.out.println(expr)
    postfix({ trig = ".sout", match_pattern = text_pattern }, fmt("System.out.println({});", {
        match_content(1)
    })),

    -- .return -> return expr;
    postfix({ trig = ".return", match_pattern = text_pattern }, fmt("return {};", {
        match_content(1)
    })),
