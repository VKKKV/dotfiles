...
baijiaxing_word_map = {}
with open(
    os.path.join("others/program", "百家姓.txt"), "r", encoding="utf-8"
) as dict_file:
    for line in dict_file:
        line = line.strip()
        if line.startswith("#"):
...
word_freq_map = {}
with open(
    os.path.join("cn_dicts_dazhu", "zhihu_deal_sort_merge.txt"), "r", encoding="utf-8"
) as dict_file:
    ...
    with open(yaml_file_path, "r", encoding="utf-8") as dict_file:
        for line in dict_file:
            ...
            if character in word_freq_map:
                ...
                if (
                    character in need_to_reduce_freq_word_map
                    or character + "\t" + encoding in need_to_reduce_freq_word_map
                ):
                    freq = freq // 10
