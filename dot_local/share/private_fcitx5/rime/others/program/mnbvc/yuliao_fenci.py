...
if __name__ == "__main__":
    for file_name in scan_file(os.path.expanduser(MNBVC_SOURCE_TXT_PATH)):
        ...
        # 遍历排序后的结果
        sorted_words = heapq.nlargest(
            len(word_counter), word_counter, key=word_counter.get
        )
        with open(
            "{save_path}/zhihu_deal_sort{i}.txt".format(
                save_path=MNBVC_SOURCE_TXT_PATH, i=number
            ),
            "w",
            encoding="utf-8",
        ) as write_file:
